    ...
    public void bindData(PictureTextExpressAd baseAd) {
        ...
        if (mAdVideo != null) {
            mPictureTextAdRootView.post(() -> {
                int measuredWidth = mPictureTextAdRootView.getMeasuredWidth() - DensityUtil.dip2px(DemoApplication.sContext, 48);
    ...
    private void registerViewClickProxy(){
        ...
        if (adBrandAutoSizeLayout != null) {
            adBrandAutoSizeLayout.setOnClickListener(view ->
                    Toast.makeText(context, "包含品牌logo，品牌名称，广告标识，下载按钮，负反馈按钮", Toast.LENGTH_SHORT).show());
        }
        ...
        if (adFlagView != null) {
            adFlagView.setOnClickListener(view ->
                    Toast.makeText(context, "广告标签", Toast.LENGTH_SHORT).show());
        }
        if (adFlagCloseView != null) {
            adFlagCloseView.setOnClickListener(view ->
                    Toast.makeText(context, "广告标签-关闭", Toast.LENGTH_SHORT).show());
        }
